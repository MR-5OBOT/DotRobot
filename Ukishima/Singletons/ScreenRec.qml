 ...
 * `$XDG_CACHE_HOME/ukishima/rec-thumbs`, skipping clips already cached) and only
...
Singleton {
    ...
    readonly property string thumbDir: (Quickshell.env("XDG_CACHE_HOME") || (home + "/.cache")) + "/ukishima/rec-thumbs/"
    readonly property string thumbScript: Config.hyprPath("scripts", "rec-thumbs.sh")
    ...
    Process {
        ...
        onExited: function(exitCode) {
            ...
            if (exitCode !== 0) {
                ...
                failProc.command = ["notify-send", "-a", "Ukishima", "-u", "critical",
    ...
    Process {
        id: savedProc
        command: ["notify-send", "-a", "Ukishima", "Recording saved",
