 ...
 * flags (~/.local/state/island/events.json). The in-memory `events` is the
...
Singleton {
    ...
    readonly property string stateDir: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/island"
    ...
    function minutesOf(hhmm) {
        ...
        return p.length >= 2 ? Number(p[0]) * 60 + Number(p[1]) : -1;
    }

    function checkReminders() {
        var now = new Date();
        var today = Qt.formatDateTime(now, "yyyy-MM-dd");
        var nowMin = now.getHours() * 60 + now.getMinutes();
        var keys = root.reminded.date === today ? root.reminded.keys.slice() : [];
        var changed = root.reminded.date !== today;
        var list = root.forDate(today);
        for (var i = 0; i < list.length; i++) {
            var e = list[i];
            var key = e.id + "@" + today;
            if (keys.indexOf(key) >= 0)
                continue;
            var timed = e.time && e.time.length > 0;
            var start = timed ? root.minutesOf(e.time) : 8 * 60;
            if (start < 0 || nowMin < start)
                continue;
            keys.push(key);
            changed = true;
            if (timed && nowMin - start > 10)
                continue;
    ...
    Component.onCompleted: {
        reloadEvents();
        try {
            var r = JSON.parse(remindedFile.text());
            if (r && typeof r.date === "string" && Array.isArray(r.keys))
                root.reminded = r;
        } catch (e) {}
        checkReminders();
    }
    ...
    FileView {
        id: remindedFile
        path: root.stateDir + "/reminded.json"
        blockLoading: true
        printErrors: false
    }
}
