...
Singleton {
    ...
    readonly property string stateFile: (Quickshell.env("XDG_STATE_HOME") || (Quickshell.env("HOME") + "/.local/state")) + "/island/nvibrant-value"
     ...
     * detection runs whenever the mixer opens: one external monitor may be
     * the only connected display. The internal backlight is probed separately.
     */
    function detect() {
