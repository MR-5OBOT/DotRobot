...
It runs next to the main quickshell config in `dotfiles/.config/quickshell` and hands a few jobs to it: the launcher, clipboard, wallpaper picker, wifi/bluetooth panel and power-profile menu all open from the island but are the main shell's widgets, drawn in the same notch style.
...
- **Panels** — calendar with events, weather, media player, mixer (volume, mic, brightness, DND, keep-awake, night light), battery with power-profile button, system monitor with a speed test, notification inbox and toasts, minimized-window stash, power menu, settings (display, font, interface).
...
The island's own updater page is gone, but keep running it from a copy outside the repo so nothing can ever touch the repo's git state:

```sh
cp -a ~/repos/DotRobot/island ~/.local/share/quickshell/island
~/.local/share/quickshell/island/launch.sh
```

`launch.sh` sets jemalloc decay options so memory stays near the live working set.
...
```sh
qs -p ~/.local/share/quickshell/island ipc call island calendar ""
qs -p ~/.local/share/quickshell/island ipc call island page "" weather
```

Handlers: `mixer`, `calendar`, `launcher`, `power`, `link`, `battery`, `sysmon`/`system`, `clipboard`, `wallpaper`, `media`, `peek`, `hide`, `unloadAll`, `page`, `minimizeWindow`, `restoreWindow`. `launcher`, `clipboard` and `wallpaper` open the main shell's widgets.
...
- state: `~/.local/state/island` (flags, events) and `~/.local/state/island-wallpaper*`
